...
function makeRequest(body) {
  return new Request('http://localhost/api/soldiers/check-id', {
    method: 'POST',
...
describe('/api/soldiers/check-id POST', () => {
  let POST;
  ...
  it('returns 400 when idNumber missing', async () => {
    mockRequireAuth.mockResolvedValue({ ok: true, uid: 'u1' });
    const res = await POST(makeRequest({}));
    expect(res.status).toBe(400);
  });

  it('returns taken false for own uid doc', async () => {
    mockRequireAuth.mockResolvedValue({ ok: true, uid: 'u1' });
    mockGet.mockResolvedValue({ docs: [{ id: 'u1' }] });
